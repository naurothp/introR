...
* NAs stellen ein häufiges Problem für Funktione in R dar:
* Prinzipiell 2 Möglichkeiten:
  + Funktionsparamter vorhanden (z.B. `na.rm=TRUE`)
...
by(diamonds, diamonds[,"cut"], function(x) lm(carat~price, data=x)$coefficients)
...
# Gundlegende statistische Verfahren
...
> TODO: Unterscheiden sich die "gute"" Diamanten preislich von "sehr guten" Diamanten?
...
tTestData <- diamonds %>%                   # %>% = then ("pipelining")
...
```{r eval=FALSE}
myRegression <- lm(price~carat + as.numeric(clarity) + as.numeric(color), data = diamonds)
summary(myRegression)
```

# Gundlegende Programmierkenntnisse
...
## Potentielle Themen
  ...
  * __Varianzanalyse__: `car`, `ez`, `multcomp`...
...
## Potentielle Themen 2
  ...
  * Effekstärkenberechnung / Poweranalyse
  * Metaanalytische Verfahren: z.B. `metafor`
